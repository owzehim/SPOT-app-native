...
export default function ScanPage() {
  ...
  async function handleScan({ data: rawValue }) {
    ...
    try {
      ...
      if (result.success) {
        ...
        setState(STATE.ERROR);
        setErrorMsg(
          result.message || '할인을 적용할 수 없습니다. 다시 시도해주세요.'
        );
      }
    } catch (err) {
      setState(STATE.ERROR);
  ...
  // 웹용 QRScanner는 문자열만 넘기므로 expo-camera 형식에 맞게 래핑
  const handleWebScan = (decodedText) => {
    handleScan({ data: decodedText });
  };
  ...
  // Permission not yet determined (native 전용)
  if (Platform.OS !== 'web' && !permission) {
    return <View style={{ flex: 1, backgroundColor: '#f9fafb' }} />;
  }

  // Permission denied (native 전용)
      ...
      >
        {state === STATE.SCANNING && (
          <View style={{ width: '100%', alignItems: 'center', gap: 16 }}>
            ...
            {Platform.OS === 'web' ? (
              // 웹: PWA에서 사용하던 Html5Qrcode 기반 스캐너
              <View style={{ width: 280, maxWidth: '100%' }}>
                <QRScanner onScan={handleWebScan} />
              </View>
            ) : (
              // native: 기존 expo-camera 스캐너
              <View
                style={{
                  width: 280,
                  height: 280,
        ...
        {state === STATE.SUCCESS && (
          ...
          >
            ...
            <Text
              style={{ fontWeight: 'bold', color: '#111827', fontSize: 20 }}
            >
              할인 적용 완료!
            </Text>
            <Text
            ...
            >
              <Text style={{ fontWeight: '600' }}>{storeName}</Text>
              에서의 할인이 기록되었습니다.{'\n'}이 화면을 직원에게
              보여주세요.
            </Text>
            ...
            >
              <Text
                style={{ color: '#4b5563', fontWeight: '500', fontSize: 14 }}
              >
                홈으로 돌아가기
              </Text>
            </TouchableOpacity>
